	...
	protected function render() {
		...
		if ( empty( $cross_sells ) ) {
			if ( Plugin::instance()->editor->is_edit_mode() ) {
				printf( '<center>%s</center>', __( 'To view the <strong>Woo Cross Sells</strong>, you must add products to the cart that has cross-selling items with it.', 'essential-addons-elementor' ) ); //This is a static value no.
			}
		...
		if ( ! empty( $heading ) ) {
			$heading = sprintf( '<%1$s class="eael-woo-cross-sells-heading">%2$s</%1$s>', $settings['eael_woo_cross_sells_heading_tag'], $heading );
			echo wp_kses( $heading, HelperClass::eael_allowed_tags() );
		} ?>
        ...
        <div <?php $this->print_render_attribute_string( 'container' ); ?>>
			...
			if ( file_exists( $template ) ) {
				...
				echo '<p class="eael-no-layout-found">' . esc_html__( 'No layout found!', 'essential-addons-elementor' ) . '</p>';
			} ?>
        </div>
		<?php
	}
