...
;jQuery(document).ready(function($) {
	...
	function elist_arrange(names) {
        ...
        var filters = {
            'all'           : CCJ.rules_i18n["all"],            // 'All Website',
            'first-page'    : CCJ.rules_i18n["first-page"],     // 'Homepage',
            'contains'      : CCJ.rules_i18n["contains"],       // 'URL Contains',
            'not-contains'  : CCJ.rules_i18n["not-contains"],   // 'URL not contains',
            'equal-to'      : CCJ.rules_i18n["equal-to"],       // 'URL is equal to',
            'not-equal-to'  : CCJ.rules_i18n["not-equal-to"],   // 'URL not equal to',
            'begins-with'   : CCJ.rules_i18n["begins-with"],    // 'URL starts with',
            'ends-by'       : CCJ.rules_i18n["ends-by"],        // 'URL ends by',
            'wp-conditional': CCJ.rules_i18n["wp-conditional"], // 'WP Conditional Tag',
        };
	...
	function elist_add(name, value, inputs, row) {
		...
		field = ('' === field)? [] : JSON.parse(field);
		if (!(field instanceof Array) || ( field.length > 24 && window.location.search.indexOf('limit_nr_rules=no') === -1 ) )
			return;
	...
	$('.wplnst-elist').on('click', '.wplnst-elist-close-link', function() {
		...
		if ('' !== field) {
			...
			if (field instanceof Array) {
				...
				$('.wplnst-' + name + '-' + index).hide('slow', function() { $(this).remove(); });
				
				if (0 === field_new.length)
					$('#' + 'wplnst-elist-' + name).hide( 300 );
			}
		}
	...
	$('#wplnst-af-new-type').change(function() {
        ...
        if( selected == 'all' || selected == 'first-page' ) {
            $('#wplnst-af-new').hide(300);
        } else {
            $('#wplnst-af-new').show(300).focus();
        }
	});
