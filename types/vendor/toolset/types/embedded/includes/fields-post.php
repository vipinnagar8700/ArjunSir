...
function wpcf_admin_post_init( $post ) {
	...
	// Get groups.
	//
	// We provide the post type in case we're on the Add New page, in which case we'll get
	// the groups assigned to the whole post type, not specifically for the new post (which doesn't
	// yet exist at this point).
	$groups = wpcf_admin_post_get_post_groups_fields( $post, 'group', $post_type );
...
function wpcf_add_meta_boxes( $post_type, $post ) {
	...
	foreach ( $groups as $group ) {
		$field_group_object = new Toolset_Field_Group_Post( get_post( $group['id'] ) );
		...
		if ( ! empty( $group['fields'] ) && empty( $only_preview ) ) {
			if ( defined( 'WPTOOLSET_FORMS_VERSION' ) ) {
				...
				$repeatable_groups_count = 0;

				foreach ( $group['fields'] as $field ) {
					// In PHP 5.3 the isset clause returns true if it is a string!!!
					if ( ! is_array( $field ) || ! isset( $field['id'] ) ) {
						...
						if ( $repeatable_group = $repeatable_group_service->get_object_from_prefixed_string( $field ) ) {
							...
							$field_slugs = $repeatable_group->get_field_slugs();
							if ( ! empty( $field_slugs ) ) {
								if( $repeatable_groups_count === 0 ) {
									// render templates for horizontal / vertical view
									// this is only required once, no matter how many groups are on the screen
									$rfg_service = new Types_Field_Group_Repeatable_Service();
									$view = $rfg_service->get_view_backend_post( get_current_user_id(), get_post_type() );
									$view->prepare();
								}

								$group['html'][] = $view->render( $repeatable_group );
								$repeatable_groups_count++;
							}
						}
						continue;
					}
					...
					if ( $field['type'] === 'post' ) {
						if ( ! isset( $field_service ) || ! isset( $post_reference_factory ) ) {
							$field_service = \Types_Field_Service_Store::get_instance()->get_service( true );
							$post_reference_factory = new Types_Field_Type_Post_Factory();
						}

						$field_object = $field_service->get_field( new Types_Field_Gateway_Wordpress_Post(), $field['id'], get_the_ID() );
						if( ! $field_object ) {
						...
						if( ! isset( $view_post_reference_field ) ) {
							$view_post_reference_field = $post_reference_factory->get_view_backend_display( $field_object );
							$view_post_reference_field->prepare();
						}
					...
					// Note that the $single argument defaults to false, ergo $meta will be always an array of metadata,
					// even for single fields.
					$meta   = get_post_meta( $post->ID, $field['meta_key'] );

					// Handle "copy" and "copy once" fields on new posts: We pre-fill the field inputs because:
					//
					// 1. Leaving them empty and later saving the empty strings as actual values would interfere with
					//    the WPML mechanism that populates such fields correctly.
					// 2. The user should see the actual content without needing to reload the page, especially
					//    when using the block editor.
					if (
						'post-new.php' === $pagenow
						&& wpcf_wpml_field_has_translation_setting(
							$field, [ CustomFieldTranslationSetting::COPY, CustomFieldTranslationSetting::COPY_ONCE ], $post
						)
					) {
						$original_post_id = WpmlService::get_instance()->get_original_post_id( $post->ID );
						if ( $original_post_id ) {
							$meta = get_post_meta( $original_post_id, $field['meta_key'] );
						}
					}
		...
		if ( ! isset( $group['__show_meta_box'] ) || $group['__show_meta_box'] != false ) {
			$field_group = Toolset_Field_Group_Post_Factory::load( $group['slug'], $force_query_by_name = true );
	...
	// Print in an easily readable way which groups we're actually rendering.
	// @see \OTGS\Toolset\Types\Page\Extension\AddOrEditPost::initialize() and setupReloadAfterSaving() in AddOrEdit.js for how this is being used
	if ( ! empty( $groups ) ) {
		add_action( 'admin_footer', static function () use ( $groups ) {
			$group_slugs = array_filter(
				array_map( static function( $group ) {
					return toolset_getarr( $group, 'slug' );
				}, $groups ),
				static function( $group_slug ) {
					return ! empty( $group_slug );
				}
			);
			printf( '<script id="types_displayed_field_group_slugs" type="application/json">%s</script>', wp_json_encode( $group_slugs ) );
		} );
	}
}
...
<tbody>' . "\n\n";
	...
	if ( ! empty( $fields ) ) {
		foreach ( $fields as $field ) {
			...
			if ( wpcf_admin_is_repetitive( $field ) ) {
			...
			} else if ( is_string( $field ) && strpos( $field, Types_Field_Group_Repeatable::PREFIX ) !== false ) {
				$repeatable_group_service = new Types_Field_Group_Repeatable_Service();
				...
				$post_field = $field_definition->instantiate( $post->ID );
				$renderer = $post_field->get_renderer( Toolset_Field_Renderer_Purpose::PREVIEW, Toolset_Common_Bootstrap::MODE_ADMIN, array() )->render();
				$html = $renderer;
			} else {
...
function wpcf_admin_post_meta_box( $post, $group, $echo = '', $open_style_editor = false ) {
	$field_group = Toolset_Field_Group_Post_Factory::load( toolset_getnest( $group, array( 'args', 'slug' ) ) );
	...
	if (
		...
		$description = toolset_getnest( $group, array( 'args', 'description' ), '' );
	...
	$group_output .= "\n\n" . '<div id="wpcf-group-metabox-id-' . esc_attr( wpcf_getnest( $group,
			array( 'args', 'slug' ), 'new-group' ) ) . '">' . "\n";
	/*
...
function wpcf_admin_post_save_post_hook( $post_ID, $post ) {
	...
	if ( ! empty( $wpcf_form_data ) ) {
		...
		$delete_attachments = apply_filters( 'wpcf_delete_attachments', false );
		$images_to_delete   = array();
		if ( $delete_attachments && isset( $_POST['wpcf'] ) && array_key_exists( 'delete-image',
				$_POST['wpcf'] ) && is_array( $_POST['wpcf']['delete-image'] )
		) {
		...
		foreach ( $wpcf_form_data as $field_slug => $field_value ) {
			...
			// Since Types 3.4.11, if a custom field is hidden conditionally, we're going to reset it upon saving.
			// The field default value (or empty value) will be used.
			$is_hidden_conditionally = ! wptoolset_form_conditional_check( $config );
			if ( $is_hidden_conditionally ) {
				$_field_value = [ '' ];
				$field_value = types_is_repetitive( $field ) ? [ '' ] : '';
			}
			...
			foreach ( $_field_value as $_k => $_val ) {
				if ( ! $is_hidden_conditionally ) {
					...
					$not_valid = is_wp_error( $validation );
			...
			// Save field
			if ( types_is_repetitive( $field ) ) {
				/** @var WPCF_Repeater $repeater */
				$repeater = $wpcf->repeater;
				$repeater->set( $post_ID, $field );
				$repeater->save( $field_value );
			} elseif ( ! $not_valid ) {
				/** @var WPCF_Field $field */
				$field_model = $wpcf->field;
				$field_model->set( $post_ID, $field );
				$field_model->save( $field_value );
			}
...
function wpcf_admin_post_add_attachment_hook( $post_ID, $post ) {
	...
	foreach ( $groups as $group ) {
		if ( isset( $group['fields'] ) ) {
			...
			if ( $form && $form->isError() ) {
				$is_not_valid = array_merge( $is_not_valid, (array) $form->get_not_valid() );
			}
		}
	}
	// Set fields
	...
	foreach ( $all_fields as $field ) {
		...
		$save_empty_setting = $field_data['data']['save_empty'];
		$field_type_definition = Toolset_Field_Type_Definition_Factory::get_instance()->load_field_type_definition( 'checkbox' );
		if ( $field_type_definition ) {
			$save_empty_setting = $field_type_definition->filter_save_empty_setting( $save_empty_setting );
		}
		if ( Toolset_Field_Definition::SAVE_EMPTY_NO === $save_empty_setting && ! $checked ) {
			continue;
		}
 ...
 * @return type
 */
function wpcf_admin_post_process_fields(
	$post = false,
	$fields = array(),
	$use_cache = true,
	$add_to_editor = true,
...
function wpcf_admin_post_process_field( $field_object ) {
	...
	if ( ! empty( $field ) ) {
		 ...
		 */
		$_element = array(
			'#type'     => isset( $field_object->config->inherited_field_type )
				? $field_object->config->inherited_field_type : $field['type'],
			'#id'       => $field_id,
			'#title'    => $field['name'],
		...
		if ( types_is_repetitive( $field ) && $context != 'post_relationship' && wpcf_wpml_field_is_copied( $field ) ) {
			if ( ! empty( $element['#options'] ) && $element['#type'] != 'select' ) {
				foreach ( $element['#options'] as $temp_key => $temp_value ) {
					$element['#options'][ $temp_key ]['#attributes']['class'] = isset( $element['#attributes']['class'] )
						? $element['#attributes']['class'] . ' wpcf-repetitive' : 'wpcf-repetitive';
				}
			} else {
		...
		if ( isset( $element['#attributes']['readonly'] ) || isset( $element['#attributes']['disabled'] ) ) {
			...
			if ( $field['type'] == 'checkboxes' ) {
				if ( isset( $element['#options'] ) ) {
					foreach ( $element['#options'] as $temp_key => $temp_value ) {
						$value                                 = isset( $temp_value['#default_value'] )
							? $temp_value['#default_value'] : $temp_value['#value'];
						$_after                                = "<input type=\"hidden\" name=\"{$temp_value['#name']}\" value=\"{$value}\" />";
						$temp_value['#after']                  = isset( $temp_value['#after'] )
							? $temp_value['#after'] . $_after : $_after;
				...
				$element['#after']                  = isset( $element['#after'] ) ? $element['#after'] . $_after
					: $_after;
				$element['#attributes']['disabled'] = 'disabled';
 ...
 *
 * @param int|false|WP_Post $post
 * @param string $context
 * @param null|string $post_type Post type, if known. It will be used to retrieve field groups for
 *     a post type in case the particular post is not provided.
 *
 * @return mixed
 * @deprecated Use Toolset_Field_Group_Post_Factory::get_groups_for_element() instead.
 * @since the dawn of time
 */
function wpcf_admin_post_get_post_groups_fields( $post = false, $context = 'group', $post_type = null ) {
	...
	if ( null !== $element ) {
		...
		// There's no particular post but we at least know what post type we're dealing with.
		// This ought to happen on the Add New page.
		$groups_for_post_type = $group_factory->get_groups_by_post_type( $post_type );

		$selected_group_ids = array_map( static function( Toolset_Field_Group_Post $field_group ) {
			return $field_group->get_id();
		}, $groups_for_post_type );

		// We don't have enough information to decide about this.
		$require_page_reload_after_saving = false;
		$group_display_results = null;
	} else {
