...
function wpcf_wpml_field_is_copied( $field, $post = null ) {
	return wpcf_wpml_field_has_translation_setting( $field, CustomFieldTranslationSetting::COPY, $post );
}

function wpcf_wpml_field_has_translation_setting( $field, $translation_settings, $post = null ) {
	if (
		defined( 'ICL_SITEPRESS_VERSION' )
		&& defined( 'WPML_TM_VERSION' )
		&& (
			! defined( 'DOING_AJAX' )
			|| ( isset( $_REQUEST['action'] ) && $_REQUEST['action'] === 'types_repeatable_group' )
		)
		&& isset( $field['wpml_action'] )
		&& ! wpcf_wpml_post_is_original( $post )
		&& wpcf_wpml_have_original( $post )
	) {
		return in_array( (int) $field['wpml_action'], toolset_wraparr( $translation_settings ), true );
	}
	return false;
}
...
function wpcf_wpml_field_is_copy( $field ) {
    ...
    return isset( $field['wpml_action'] ) && (int) $field['wpml_action'] === CustomFieldTranslationSetting::COPY;
}
...
function wpcf_wpml_field_is_translated( $field ) {
    ...
    return isset( $field['wpml_action'] ) && (int) $field['wpml_action'] === CustomFieldTranslationSetting::TRANSLATE;
}
...
function wpcf_wpml_post_is_original( $post = null ) {
    if ( defined( 'ICL_SITEPRESS_VERSION' ) ) {
        ...
        if ( $pagenow === 'post-new.php' ) {
            ...
            if ( array_key_exists( $post_type, $sitepress->get_translatable_documents() ) ) {
            ...
            if (
				! empty( $post->ID )
				&& array_key_exists( $post->post_type, $sitepress->get_translatable_documents() )
			) {
