...
class Types_Image_View
{
    ...
    private static function resizeImg( $img, $args = array() ) {
        ...
        // regenerated images other than gif and png will always be jpg, so change the extension now before checking if the resized image already exists
        $extension = ( in_array( $imgData->pathinfo['extension'], array('gif','png','jpeg') ) ) ? $imgData->pathinfo['extension'] : 'jpg';
        $croppedImg = "{$path}{$basename}-wpcf_{$suffix}.{$extension}";
        if ( !$args['clear_cache'] ) {
        ...
        if ( !is_resource( $imgRes ) && ! $imgRes instanceof \GdImage ) {
            return new WP_Error( __CLASS__ . '::' . __METHOD__,
...
class Types_Image_Utils
{
    ...
    /**
     * Custom mb_basename function
     * (because basename is brittle, locale dependent)
	...
	public static function getAbsPath( $imgUrl ) {
        ...
        $basename = self::mb_basename( $imgUrl );
		...
		if ( $upload_dir
			...
			if (
				...
				if ( isset( $multisite_parsed[1] ) ) {
					$image_absolute_path = $upload_dir['basedir'] . DIRECTORY_SEPARATOR . dirname( $multisite_parsed[1] ) . DIRECTORY_SEPARATOR . $basename;
				}
			}
		}
		...
		if ( empty( $image_absolute_path ) ) {
			...
			$image_absolute_path .= DIRECTORY_SEPARATOR . $basename;
		}
