 ...
 * $wpcf->repeater->set(...);
 *
 * @property mixed meta_object
 * @property string order_meta_name
 * @since Types 1.2
 */
class WPCF_Repeater extends WPCF_Field
{
     ...
     */
    public function set( $post, $field ) {
    ...
    public function save( $value = null ) {
        ...
        $this->delete_postmeta_by_key( $this->slug );

        // Allow $data to replace $_POST
		if ( isset( $_POST['wpcf'][ $this->cf['slug'] ] ) && is_null( $value ) ) {
			$value = $_POST['wpcf'][ $this->cf['slug'] ];
        }
        ...
        if ( !empty( $value ) ) {
            ...
            // Save order
            if ( !empty( $mids ) ) {
				// Unfortunately, we can't use just update_post_meta() here. Until Types 3.4.9,
				// the postmeta was always added, which means that there may be a lot of
				// redundant garbage in the database. This way, we ensure only a single postmeta
				// value is stored for the current post.
				delete_post_meta( $this->post->ID, $this->order_meta_name );
				add_post_meta( $this->post->ID, $this->order_meta_name, $mids );
            }
     ...
     */
    public function get_fields_form() {
        $form = array();
        $form_id = $this->cf['id'];
     ...
     * @return string
     */
    public function get_field_form( $meta_value = null, $meta_id = null ) {
...
//            $_loop = true;
     ...
     */
    public function _set_form_count() {
        if ( $this->index === 0 ) {
	 ...
	 * @return int|WP_Error
	 */
	private function delete_postmeta_by_key( $meta_key ) {
		global $wpdb;
		...
		if ( $affected_rows === false ) {
			return new WP_Error(
				'wpcf_repeater_delete_field',
				sprintf( 'Repeater failed deleting post_ID: %d; meta_key: %s', $this->post->ID, $meta_key )
			);
		}
...
class WPCF_Termmeta_Repeater extends WPCF_Termmeta_Field
{
     ...
     * Calls parent set func.
     */
    public function set( $term_id, $field ) {
    ...
    public function save( $data = null ) {
        ...
        if ( !empty( $data ) ) {
            ...
            // Save order
            if ( !empty( $mids ) ) {
                update_term_meta( $this->term_id, $this->order_meta_name, $mids );
            }
     ...
     * @todo Make more distinction between $field_form and $form_field
     */
    public function get_fields_form( $is_profile = '' ) {
        $form = array();
        $form_id = $this->cf['id'];
        $unique_id = wpcf_unique_id( serialize( $this->cf ) );
