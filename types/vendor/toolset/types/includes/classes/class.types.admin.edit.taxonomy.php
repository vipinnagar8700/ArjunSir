 ...
 <a href="#visibility" class="cancel-post-visibility hide-if-no-js button-cancel">Cancel</a>
    ...
    public function box_options()
    {
        ...
        $form['query_var'] = array(
            ...
            '#title' => 'query_var',
            '#description' => sprintf(
				__( 'Enable, disable or customize the variable to reference and query posts on specific taxonomy terms in the frontend. Check the %1$sdocumentation%2$s.', 'wpcf' ),
				'<a href="https://developer.wordpress.org/reference/functions/register_taxonomy/#arguments">',
				'</a>'
			)
			. '<br />'
			. sprintf(
				__( 'Default: true - set to $taxonomy%1$s.', 'wpcf' ),
				( false === toolset_getget( 'wpcf-tax', false ) ? '' : ( ' (mysite.com/?' . toolset_getget( 'wpcf-tax' ) . '=term_slug)' ) )
			),
