...
class Toolset_Relationship_Driver extends Toolset_Relationship_Driver_Base {
	...
	public function create_association( $parent_source, $child_source, $args = array() ) {
		...
		$potential_association_query = $this->database_layer_factory->potential_association_query(
		...
		try {
			...
			$updated_association = $this->database_layer_factory->association_persistence()->insert_association( $association );
		} catch( Exception $e ) {
	 ...
	 */
	public function create_intermediary_post_type( $new_slug_candidate = null, $show_in_menu = false ) {
		...
		if( null !== $post_type_slug && $post_type_repository->has( $post_type_slug ) ) {
			...
			$this->set_intermediary_post_type( $post_type_repository->get( $post_type_slug ), $show_in_menu );
			return $post_type_slug;
		}
		...
		$this->set_intermediary_post_type( $post_type, $show_in_menu );

		$post_type->set_is_public( true );
		$post_type->set_show_in_menu( $show_in_menu );
	 ...
	 * @param bool $show_in_menu If the intermediary post type should be visible in the admin menu.
	 */
	public function set_intermediary_post_type( IToolset_Post_Type_From_Types $post_type = null, $show_in_menu = false ) {
		$post_type_repository = Toolset_Post_Type_Repository::get_instance();
	...
	public function delete_association( $association ) {
		return $this->database_layer_factory->association_persistence()->delete_association( $association );
	}
