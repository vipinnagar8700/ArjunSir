...
use OTGS\Toolset\Common\Field\Renderer\Rest\Attachment;
use OTGS\Toolset\Common\Field\Renderer\Rest\Raw;
use OTGS\Toolset\Common\Field\Renderer\Rest\Skype;
use OTGS\Toolset\Common\Field\Renderer\RfgTranslationPreview\Checkbox;
use OTGS\Toolset\Common\Field\Renderer\RfgTranslationPreview\Checkboxes;
use OTGS\Toolset\Common\Field\Renderer\RfgTranslationPreview\Date;
use OTGS\Toolset\Common\Field\Renderer\RfgTranslationPreview\DefaultRenderer;
use OTGS\Toolset\Common\Field\Renderer\RfgTranslationPreview\Radio;
use OTGS\Toolset\Common\Field\Renderer\RfgTranslationPreview\Select;
 ...
 * @since 2.0
 */
class Toolset_Field_Type_Definition implements CustomFieldTypeDefinition {
				...
				switch ( $this->get_slug() ) {
					...
					case Toolset_Field_Type_Definition_Factory::IMAGE:
						return new Attachment(
							$field,
							new Attachments( new TypesGuidIdGateway(), \OTGS\Toolset\Common\WPML\WpmlService::get_instance() )
						);
					...
					case Toolset_Field_Type_Definition_Factory::SKYPE:
						return new Skype( $field );
					default:
						return new Raw( $field );
				}
				break;

			case Toolset_Field_Renderer_Purpose::RFG_TRANSLATION_PREVIEW:
				switch( $this->get_slug() ) {
					...
					default:
						return new DefaultRenderer( $field );
				}
	...
	/**
	 * Filter the 'save_empty' option of a field definition by the field type.
	 *
	 * For some field types, we always enforce saving empty values, for others we never do it, and sometimes
	 * it's optional.
	 *
	 * IMPORTANT: Whenever this option is used, it MUST pass through this filter (that goes especially for the legacy code
	 * that doesn't use the Toolset_Field_Definition model).
	 *
	 * @param string $value
	 * @return string
	 * @since Types 3.4.11
	...
	/**
	 * Return what should be an empty value saved to the database, if the field supports saving it.
	 *
	 * Note: This is not applied consistently across the legacy codebase, but it should become the authoritative
