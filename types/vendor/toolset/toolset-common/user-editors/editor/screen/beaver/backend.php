...
class Toolset_User_Editors_Editor_Screen_Beaver_Backend
	...
	public function initialize() {
		...
		// Registers the Beaver Builder editor as one of the allowed default user editors for Content Templates.
		add_filter( 'wpv_filter_wpv_default_user_editor', array( $this, 'register_beaver_as_default_user_editor_option' ) );
	}
	...
	*/
	public function prevent_nested( $post ) {
		...
		if ( in_array( $post->post_type, $beaver_post_types ) ) {
			$post_has_ct	= apply_filters( 'wpv_content_template_for_post', 0, $post );
			$ct_has_beaver	= false;
			if ( $post_has_ct ) {
	 ...
	 * @return array
	 */
	public function register_beaver_as_default_user_editor_option( $default_editors ) {
		array_push( $default_editors, Toolset_User_Editors_Editor_Beaver::BEAVER_SCREEN_ID );
